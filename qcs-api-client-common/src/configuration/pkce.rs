...
pub(in crate::configuration) mod tests {
    ...
    ///
    /// IMPORTANT: for now, mark your tests with `#[serial_test::serial(oauth2_test_server)]` to ensure that
    /// they run serially, as the OAuth test server does not (yet) support wildcarding the redirect URI, so we
    /// cannot yet allow the `pkce_login` listener to bind to any port.
