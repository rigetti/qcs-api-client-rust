...
/// InternalCreateEventBillingPriceRequest : Create an association between a QCS service product and a `BillingPrice`. This price can be optionally scoped to a particular `accountId` and `quantumProcessorId`.
...
impl InternalCreateEventBillingPriceRequest {
    /// Create an association between a QCS service product and a `BillingPrice`. This price can be optionally scoped to a particular `accountId` and `quantumProcessorId`.
    pub fn new(
