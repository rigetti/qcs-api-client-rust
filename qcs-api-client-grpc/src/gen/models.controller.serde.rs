...
impl serde::Serialize for RiverlaneMetadata {
    #[allow(deprecated)]
    fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        ...
        let mut struct_ser = serializer.serialize_struct("models.controller.RiverlaneMetadata", len)?;
        if !self.ds2_configuration_data.is_empty() {
...
impl<'de> serde::Deserialize<'de> for RiverlaneMetadata {
    #[allow(deprecated)]
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        ...
        struct GeneratedVisitor;
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            type Value = RiverlaneMetadata;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("struct models.controller.RiverlaneMetadata")
            }

            fn visit_map<V>(self, mut map_: V) -> std::result::Result<RiverlaneMetadata, V::Error>
                where
                    V: serde::de::MapAccess<'de>,
            {
                ...
                Ok(RiverlaneMetadata {
                    ds2_configuration_data: ds2_configuration_data__.unwrap_or_default(),
                })
            }
        }
        deserializer.deserialize_struct("models.controller.RiverlaneMetadata", FIELDS, GeneratedVisitor)
    }
}
